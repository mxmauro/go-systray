...
type winTray struct {
	...
	loadedImages   map[uint64]windows.Handle
...
func (t *winTray) setIcon(iconBytes []byte) error {
	...
	h, err := t.loadIconFrom(iconBytes)
	if err != nil {
		return err
	}
...
func (t *winTray) initInstance() error {
	...
	t.loadedImages = make(map[uint64]windows.Handle)
...
func (t *winTray) loadIconFrom(iconBytes []byte) (windows.Handle, error) {
	if !wt.isReady() {
	...
	hashCalc := fnv.New64()
	_, _ = hashCalc.Write(iconBytes)
	imageHash := hashCalc.Sum64()

	// Save and reuse handles of loaded images
	t.muLoadedImages.RLock()
	h, ok := t.loadedImages[imageHash]
	t.muLoadedImages.RUnlock()
	if !ok {
		var res uintptr

		tmpFile, err := os.CreateTemp("", "systray-*.tmp")
		if err == nil {
			var filenameStr []uint16

			filename := tmpFile.Name()

			filenameStr, err = syscall.UTF16FromString(filename)
			if err == nil {
		...
		if !ok {
			...
			t.loadedImages[imageHash] = h
		} else {
			pDestroyIcon.Call(res)
		}
...
func (item *MenuItem) SetIcon(iconBytes []byte) {
	h, err := wt.loadIconFrom(iconBytes)
	if err != nil {
